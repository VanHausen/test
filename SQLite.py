...
def export_to_sqlite():
    ...
    cursor.execute('CREATE TABLE IF NOT EXISTS tablesx (id int, company text, fact text, forecast text)')

    # 2. Работа c xlsx файлом

    # Читаем файл и лист1 книги excel
    file_to_read = openpyxl.load_workbook('/Desktop/tablesx.xlsx', data_only=True)
    sheet = file_to_read['Sheet_1']
    ...
    # 3. Запись в базу и закрытие соединения
        ...
        cursor.execute("INSERT INTO tablesx VALUES (?, ?, ?, ?);", (data[0], data[1], data[2], data[3]))
...
def clear_base():
    ...
    cursor.execute("DELETE FROM tablesx")
    connect.commit()
    connect.close()
